...
# r = np.linspace(0, 1, n)
# th = np.linspace(-1*np.pi, np.pi, n)
# # X = np.stack((x1, x2), axis = 1) 
# Xdr = np.array(np.meshgrid([0,1,2], [1,2,3], r, th, )).T.reshape(-1,4)
# ydr = vibrating_drum(Xdr)
# fig = plt.figure()
# ax = fig.add_subplot(projection='3d')
# x1 = Xdr[:,2] * np.cos(Xdr[:,3])
# x2 = Xdr[:,2] * np.sin(Xdr[:,3])
# ax.scatter(Xdr[Xdr[:,0]==0,2], Xdr[Xdr[:,0]==0,3], ydr[Xdr[:,0]==0])
# ax.scatter(x1,x2, ydr)
